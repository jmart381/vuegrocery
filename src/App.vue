...
const groceries = useStorage('groceries', [])
  ...
  const deleteGrocery = id => {
    ...
    confetti({particleCount: 30, speed: 100, origin: {y:1}})
  }
</script>

<template>
<main>
  <h1 class="title">📝 Vue My Grocery List 📝</h1>
  ...
  <h3> Items in list: {{groceries.length  }}</h3>
...
main {
  ...
  form 
  {
    ...
    button{
      @apply bg-accent text-background p-2 text-2xl font-bold rounded-r-md outline-none;
      &:hover{
        @apply bg-purplish;
      }
    }
